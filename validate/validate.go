...
func (v *Validator) CheckHooks() (errs error) {
	...
	if v.spec.Hooks != nil {
		errs = errors.Join(errs, v.checkEventHooks("prestart", v.spec.Hooks.Prestart, v.HostSpecific))
...
func (v *Validator) CheckProcess() (errs error) {
	...
	for _, env := range process.Env {
		if !envValid(env) {
			errs = errors.Join(errs, fmt.Errorf("env %q should be in the form of 'key=value'. The left hand side must consist solely of letters, digits, and underscores '_'", env))
		}
	}
		...
		if filepath.IsAbs(process.Args[0]) && v.spec.Root != nil {
			...
			if os.IsNotExist(err) {
				...
				if m.IsDir() || m&0o111 == 0 {
					errs = errors.Join(errs, fmt.Errorf("arg %q is not executable", process.Args[0]))
				}
			}
		}
	}
	...
	if v.platform == "linux" || v.platform == "solaris" {
		errs = errors.Join(errs, v.CheckRlimits())
	}

	if v.platform == "linux" {
		if v.spec.Process.Capabilities != nil {
			errs = errors.Join(errs, v.CheckCapabilities())
		}
...
func (v *Validator) CheckCapabilities() (errs error) {
	if v.platform != "linux" {
		errs = errors.Join(errs, fmt.Errorf("For %q platform, the configuration structure does not support process.capabilities", v.platform))
		return
	}
	...
	for capability, owns := range caps {
		if err := CapValid(capability, v.HostSpecific); err != nil {
			errs = errors.Join(errs, fmt.Errorf("capability %q is not valid, man capabilities(7)", capability))
		}
		...
		if effective && !permitted {
			errs = errors.Join(errs, fmt.Errorf("effective capability %q is not allowed, as it's not permitted", capability))
		}
		if ambient && !(permitted && inheritable) {
			errs = errors.Join(errs, fmt.Errorf("ambient capability %q is not allowed, as it's not permitted and inheribate", capability))
		}
	}
...
func (v *Validator) CheckRlimits() (errs error) {
	if v.platform != "linux" && v.platform != "solaris" {
		errs = errors.Join(errs, fmt.Errorf("For %q platform, the configuration structure does not support process.rlimits", v.platform))
		return
	}
	...
	for index, rlimit := range process.Rlimits {
		...
		errs = errors.Join(errs, v.rlimitValid(rlimit))
	}
...
func (v *Validator) CheckMounts() (errs error) {
	...
	for i, mountA := range v.spec.Mounts {
		if supportedTypes != nil && !supportedTypes[mountA.Type] {
			errs = errors.Join(errs, fmt.Errorf("unsupported mount type %q", mountA.Type))
		}
...
func (v *Validator) CheckPlatform() (errs error) {
	...
	if v.platform != "linux" && v.platform != "solaris" && v.platform != "windows" {
		errs = errors.Join(errs, fmt.Errorf("platform %q is not supported", v.platform))
		return
	}

	if v.HostSpecific && v.platform != runtime.GOOS {
		errs = errors.Join(errs, fmt.Errorf("platform %q differs from the host %q, skipping host-specific checks", v.platform, runtime.GOOS))
...
func (v *Validator) CheckLinuxResources() (errs error) {
	...
	if r.Memory != nil {
		if r.Memory.Limit != nil && r.Memory.Swap != nil && uint64(*r.Memory.Limit) > uint64(*r.Memory.Swap) {
			errs = errors.Join(errs, fmt.Errorf("minimum memoryswap should be larger than memory limit"))
		}
		if r.Memory.Limit != nil && r.Memory.Reservation != nil && uint64(*r.Memory.Reservation) > uint64(*r.Memory.Limit) {
			errs = errors.Join(errs, fmt.Errorf("minimum memory limit should be larger than memory reservation"))
		}
	}
	if r.Network != nil && v.HostSpecific {
		...
		for _, prio := range r.Network.Priorities {
			...
			if !exist {
				errs = errors.Join(errs, fmt.Errorf("interface %s does not exist currently", prio.Name))
			}
		}
	}
	for index := 0; index < len(r.Devices); index++ {
		...
		case "a", "b", "c", "":
		default:
			errs = errors.Join(errs, fmt.Errorf("type of devices %s is invalid", r.Devices[index].Type))
		}
		...
		for i := 0; i < len(access); i++ {
			...
			case 'r', 'w', 'm':
			default:
				errs = errors.Join(errs, fmt.Errorf("access %s is invalid", r.Devices[index].Access))
				return
			}
		}
	}
...
func (v *Validator) rlimitValid(rlimit rspec.POSIXRlimit) (errs error) {
	if rlimit.Hard < rlimit.Soft {
		errs = errors.Join(errs, fmt.Errorf("hard limit of rlimit %s should not be less than soft limit", rlimit.Type))
	}
	...
	if v.platform == "linux" {
		...
		errs = errors.Join(errs, specerror.NewError(specerror.PosixProcRlimitsTypeValueError, fmt.Errorf("rlimit type %q may not be valid", rlimit.Type), v.spec.Version))
...
func checkMandatoryUnit(field reflect.Value, tagField reflect.StructField, parent string) (errs error) {
	...
	case reflect.Ptr:
		if mandatory && field.IsNil() {
			errs = errors.Join(errs, fmt.Errorf("'%s.%s' should not be empty", parent, tagField.Name))
		}
	case reflect.String:
	...
	for i := 0; i < objT.NumField(); i++ {
		...
		if isStructPtr(t) && objV.Field(i).IsNil() {
			if !strings.Contains(objT.Field(i).Tag.Get("json"), "omitempty") {
				errs = errors.Join(errs, fmt.Errorf("'%s.%s' should not be empty", objT.Name(), objT.Field(i).Name))
			}
		} else if (isStruct(t) || isStructPtr(t)) && objV.Field(i).CanInterface() {
			errs = errors.Join(errs, checkMandatory(objV.Field(i).Interface()))
		} else {
			errs = errors.Join(errs, checkMandatoryUnit(objV.Field(i), objT.Field(i), objT.Name()))
		}
