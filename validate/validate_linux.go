...
func (v *Validator) CheckLinux() (errs error) {
	...
	for index := 0; index < len(v.spec.Linux.Namespaces); index++ {
		...
		if ns.Path != "" && !osFilepath.IsAbs(v.platform, ns.Path) {
			errs = errors.Join(errs, specerror.NewError(specerror.NSPathAbs, fmt.Errorf("namespace.path %q is not an absolute path", ns.Path), rspec.Version))
		}

		tmpItem := nsTypeList[ns.Type]
		tmpItem.num = tmpItem.num + 1
		if tmpItem.num > 1 {
			errs = errors.Join(errs, specerror.NewError(specerror.NSErrorOnDup, fmt.Errorf("duplicated namespace %q", ns.Type), rspec.Version))
		}
	...
	if (len(v.spec.Linux.UIDMappings) > 0 || len(v.spec.Linux.GIDMappings) > 0) && !nsTypeList[rspec.UserNamespace].newExist {
		errs = errors.Join(errs, errors.New("the UID/GID mappings requires a new User namespace to be specified as well"))
	}

	for k := range v.spec.Linux.Sysctl {
		if strings.HasPrefix(k, "net.") && !nsTypeList[rspec.NetworkNamespace].newExist {
			errs = errors.Join(errs, fmt.Errorf("sysctl %v requires a new Network namespace to be specified as well", k))
		}
		if strings.HasPrefix(k, "fs.mqueue.") {
			if !nsTypeList[rspec.MountNamespace].newExist || !nsTypeList[rspec.IPCNamespace].newExist {
				errs = errors.Join(errs, fmt.Errorf("sysctl %v requires a new IPC namespace and Mount namespace to be specified as well", k))
			}
		}
	}

	if v.platform == "linux" && !nsTypeList[rspec.UTSNamespace].newExist && v.spec.Hostname != "" {
		errs = errors.Join(errs, fmt.Errorf("on Linux, hostname requires a new UTS namespace to be specified as well"))
	}
	...
	for index := 0; index < len(v.spec.Linux.Devices); index++ {
		...
		if !deviceValid(device) {
			errs = errors.Join(errs, fmt.Errorf("device %v is invalid", device))
		}

		if _, exists := devList[device.Path]; exists {
			errs = errors.Join(errs, fmt.Errorf("device %s is duplicated", device.Path))
		} else {
			...
			if os.IsNotExist(err) {
				...
				if !ok {
					errs = errors.Join(errs, specerror.NewError(specerror.DevicesAvailable,
				...
				if devType != device.Type || (devType == "c" && device.Type == "u") {
					errs = errors.Join(errs, specerror.NewError(specerror.DevicesFileNotMatch,
	...
	if v.spec.Linux.Resources != nil {
		errs = errors.Join(errs, v.CheckLinuxResources())
	}
	...
	if v.spec.Linux.MountLabel != "" {
		if err := label.Validate(v.spec.Linux.MountLabel); err != nil {
			errs = errors.Join(errs, fmt.Errorf("mountLabel %v is invalid", v.spec.Linux.MountLabel))
		}
	}
